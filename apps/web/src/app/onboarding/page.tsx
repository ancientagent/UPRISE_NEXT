...
export default function OnboardingPage() {
  ...
  const [missingMusicCommunityRequest, setMissingMusicCommunityRequest] = useState('');
  const [missingMusicCommunitySubmitting, setMissingMusicCommunitySubmitting] = useState(false);
  const [missingMusicCommunityMessage, setMissingMusicCommunityMessage] = useState<string | null>(
    null
  );
  const [missingMusicCommunityError, setMissingMusicCommunityError] = useState<string | null>(null);
  ...
  const handleSceneContinue = async () => {
    ...
    if (token) {
      try {
        const response = await api.post<{ pioneer?: boolean; sceneId?: string | null }>(
  ...
  const handleGpsRequest = async () => {
    ...
    if (!navigator.geolocation) {
      ...
      setGpsError(
        'GPS is not available on this device. You can still continue without voting access.'
      );
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        if (detected?.city && detected?.state) {
          ...
          setManualLocationMode(true);
          setLocationError(
            'Location detection failed. Enter your city/state manually to continue.'
          );
        }
  ...
  const handleMissingMusicCommunityRequest = async () => {
    const requestedName = missingMusicCommunityRequest.trim();
    if (!requestedName || !city.trim() || !state.trim() || !token) {
      return;
    }

    setMissingMusicCommunitySubmitting(true);
    setMissingMusicCommunityMessage(null);
    setMissingMusicCommunityError(null);

    try {
      const response = await api.post<MusicCommunityRequestResponse>(
        '/onboarding/music-community-requests',
        {
          requestedName,
          city: city.trim(),
          state: state.trim(),
      ...
      const signals = response.data?.reviewSignals;
      setMissingMusicCommunityMessage(
        signals
          ? `Request saved for review. Current signals: ${signals.distinctRequesterCount} requester(s) across ${signals.distinctCityCount} city context(s).`
          : 'Request saved for review.'
      );
      setMissingMusicCommunityRequest('');
    } catch {
      setMissingMusicCommunityError('Unable to save this request right now.');
    } finally {
      setMissingMusicCommunitySubmitting(false);
    }
  };
          ...
          <div className="mb-10 flex flex-wrap items-center gap-3">
            {steps.map((label, index) => (
              <div key={label} className="flex items-center gap-3">
                ...
                <span className={`text-sm ${index <= step ? 'text-black' : 'text-black/40'}`}>
                  {label}
                </span>
          ...
          {step === 0 && (
            <section className="rounded-3xl border border-black/10 bg-white/80 p-8 shadow-sm">
              ...
              <p className="mt-2 text-sm text-black/60">
                Your Home Scene is your local music scene of choice. If you don&apos;t have one,
                select a music community you are most interested in exploring.
              </p>

              {showGpsPrompt && (
                <div className="mt-5 rounded-2xl border border-black/10 bg-black/[0.03] p-4">
                  <p className="text-sm font-medium text-black">
                    Allow UPRISE to use GPS to verify location?
                  </p>
              ...
              {!manualLocationMode && city.trim() && state.trim() ? (
                ...
                <div className="mt-8 grid gap-4 md:grid-cols-2">
                  <div className="flex flex-col gap-2">
                    ...
                    {cityLoading && (
                      <p className="text-xs text-black/50">Loading city suggestions...</p>
                    )}
              ...
              <div className="mt-4 rounded-2xl border border-black/10 bg-black/[0.03] p-4">
                <p className="text-sm font-medium text-black">Request a missing music community</p>
                <p className="mt-1 text-xs text-black/60">
                  This does not create a live scene or add it to the onboarding selector. Requests
                  are reviewed after repeated submissions from distinct people in distinct cities.
                </p>
                <div className="mt-3 flex flex-col gap-2 sm:flex-row">
                  <input
                    value={missingMusicCommunityRequest}
                    onChange={(event) => setMissingMusicCommunityRequest(event.target.value)}
                    className="min-w-0 flex-1 rounded-xl border border-black/10 bg-white px-4 py-3 text-sm shadow-sm"
                    placeholder="Name the missing music community"
                  />
                  <Button
                    type="button"
                    size="sm"
                    ...
                    disabled={
                      ...
                      !city.trim() ||
                      !state.trim() ||
                      !missingMusicCommunityRequest.trim() ||
                      missingMusicCommunitySubmitting
                    }
                    onClick={() => void handleMissingMusicCommunityRequest()}
                  >
                    {missingMusicCommunitySubmitting ? 'Sending...' : 'Send Request'}
                  </Button>
                </div>
                {!token ? (
                  <p className="mt-2 text-xs text-black/50">
                    Sign in to submit missing-community requests.
                  </p>
                ) : null}
                {missingMusicCommunityMessage ? (
                  <p className="mt-2 text-xs text-black/60">{missingMusicCommunityMessage}</p>
                ) : null}
                {missingMusicCommunityError ? (
                  <p className="mt-2 text-xs text-red-600">{missingMusicCommunityError}</p>
                ) : null}
              </div>
              ...
              <div className="mt-6 space-y-4">
                ...
                {isPioneer && (
                  <div className="rounded-2xl border border-black/10 bg-white p-4">
                    ...
                    <p className="mt-2 text-base text-black">
                      Your selected city is not active yet. We will keep tracking your Home Scene
                      intent while routing you through an active scene for this parent community.
                    </p>
                    ...
                    {reviewResolutionMode === 'fallback_community' && (
                      <p className="mt-2 text-sm text-black/60">
                        No active city scene was found in {state.trim()}, so review falls back to
                        the next active city scene for {approvedMusicCommunity}.
                      </p>
                    )}
                    ...
                    {stateSceneError && (
                      <p className="mt-2 text-sm text-black/50">{stateSceneError}</p>
                    )}
                  </div>
                )}
              </div>
