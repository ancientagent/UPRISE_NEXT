...
set -euo pipefail
cd "$(dirname "$0")/../.."
QUEUE=".reliant/queue/mvp-lane-e-qa-doc-review.json"
...
while true; do
  ...
  if echo "$CLAIM" | rg -q '"claimed":false'; then
    echo "[lane-e-executor.sh] no queued tasks; stopping"
    exit 0
  fi
  echo "[lane-e-executor.sh] claimed: $CLAIM"
  echo "[lane-e-executor.sh] execute claimed slice in agent session, then complete/block it"
  exit 0
done
