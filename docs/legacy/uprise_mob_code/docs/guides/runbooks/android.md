...
**⚠️ Important**: This project uses **native Windows development** (not WSL 2 bridge). WSL 2 bridge integration causes system corruption and Metro bundler failures. All development should be done in native Windows environment.

**🚨 CRITICAL**: **Jest is TEMPORARILY DISABLED** due to process explosion issues. Do NOT run Jest, test commands, or `npm test` until NVM configuration is fixed.

**⚡ NODE_OPTIONS Fix Applied (2025-10-01)**: Global NODE_OPTIONS removed from `.bashrc`. See [NODE_OPTIONS and ENV Issues Guide](../../troubleshooting/NODE_OPTIONS_AND_ENV_ISSUES.md) for details.

**✅ ENV Variables Fix Applied (2025-10-02)**:
- `react-native-config` properly linked in `android/app/build.gradle`
- `build_config_package` resource added to resolve BuildConfig lookup
- Duplicate drawable resources removed to fix build failures
- ENV variables now load correctly from `.env` and `.env.development`
...
### Jest Process Explosion (CRITICAL)
**DO NOT RUN JEST** - Jest is disabled due to infinite process spawning:
...
- **Status**: Jest temporarily disabled (`jest.config.js` → `jest.config.js.disabled`)
- **Action**: Avoid `npm test`, `jest`, or any test commands until NVM is fixed

### Metro Bundler Issues
If Metro fails to start or bundle generation fails:

1. **OpenSSL Error**: Project scripts already set NODE_OPTIONS correctly
   ```powershell
   # Use the npm script (already has NODE_OPTIONS configured):
   npm start
   ...
   **Important**: Do NOT set NODE_OPTIONS globally in shell config files. See [NODE_OPTIONS troubleshooting guide](../../troubleshooting/NODE_OPTIONS_AND_ENV_ISSUES.md).

2. **Environment Variables Not Picked Up**: `.env` changes require rebuild
   ```powershell
   # After changing .env or .env.development:
   cd android
   ...
   **Why**: react-native-config bakes ENV vars into BuildConfig at compile time, not runtime. Metro restart alone won't pick up changes. See [Metro ENV pickup guide](../../troubleshooting/NODE_OPTIONS_AND_ENV_ISSUES.md#metro-not-picking-up-env-files).

3. **"Cannot read property 'source' of undefined"**: Clear Metro cache
   ```powershell
...
4. **Track Player Warnings**: Verify stub configuration
   ```powershell
   Test-Path "stubs\react-native-track-player\package.json"
   Test-Path "stubs\react-native-track-player\index.js"
   ```

5. **Process Explosion**: Check for global NODE_OPTIONS
   ```powershell
   # Check if NODE_OPTIONS is set globally (should be empty):
   echo $env:NODE_OPTIONS

   # If set, unset it for current session:
   ...
   **If this keeps happening**: Check `~/.bashrc` (WSL) or shell config for global NODE_OPTIONS export. See [NODE_OPTIONS fix guide](../../troubleshooting/NODE_OPTIONS_AND_ENV_ISSUES.md).

### ENV Variables Not Loading (Config Object Empty)

**Symptom**: `Config.API_BASE_URL` shows as `undefined`, Config object is empty `{}`

**Root Cause**: `react-native-config` native module not properly linked or BuildConfig class not found

**Solution Applied (2025-10-02)**:

1. **Verify react-native-config is in build.gradle dependencies**:
   ```gradle
...
2. **Add build_config_package resource** (CRITICAL):
   ```gradle
   // android/app/build.gradle
   android {
       defaultConfig {
           // ... other config
           resValue "string", "build_config_package", "com.app.uprise"
       }
   }
   ```

3. **Verify dotenv.gradle is applied**:
   ```gradle
   // android/app/build.gradle (near top)
...
4. **Check PackageList includes RNCConfigPackage**:
   ```bash
   # From project root
   grep -r "RNCConfigPackage" android/app/build/
   # Should show: import com.lugg.RNCConfig.RNCConfigPackage;
   ```
...
6. **Verify in logs**:
   ```javascript
   // Should show in Metro logs:
   NativeModules.RNCConfigModule: {"API_BASE_URL": "http://...", ...}
   Config.API_BASE_URL: http://10.0.2.2:3000
   ```

**Common Mistakes**:
- ❌ Not rebuilding APK after ENV changes (Metro reload is NOT enough)
- ❌ Missing `build_config_package` resource (causes module to return null)
- ❌ Package name mismatch between buildConfigPackage and applicationId
- ❌ Installing old APK when build fails (check APK timestamp!)

### IDE Conflicts
If experiencing IDE stability issues:

1. **Stop all development processes**
   ```powershell
   Get-Process | Where-Object {$_.ProcessName -like "*metro*" -or $_.ProcessName -like "*node*"} | Stop-Process -Force
   ```

2. **Clear IDE workspace storage**
   ```powershell
   Remove-Item -Path "$env:APPDATA\Cursor\User\workspaceStorage" -Recurse -Force -ErrorAction SilentlyContinue
   ```

3. **Avoid WSL 2 bridge integration** - Use native Windows development only
...
$env:NODE_OPTIONS = "--openssl-legacy-provider"
```
Ensure the Android SDK path is visible to Gradle (set $env:ANDROID_SDK_ROOT or add `sdk.dir=...` to `android/local.properties`).
...
**Symptom**: Build fails with `Error: Duplicate resources` for drawable files

**Root Cause**: Metro auto-generates drawable resources, but manual copies exist in `src/main/res/drawable-*/`

**Solution** (Applied 2025-10-02):
```powershell
# Remove manually placed drawable directories (Metro generates these automatically)
cd android/app/src/main/res
Remove-Item drawable-hdpi, drawable-mdpi, drawable-xhdpi, drawable-xxhdpi, drawable-xxxhdpi -Recurse -Force
...
**Prevention**: Never manually copy image assets to `android/app/src/main/res/drawable-*/`. Metro bundles them automatically from the JS assets.
