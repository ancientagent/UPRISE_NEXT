...
• Every song must reference the canonical performer id (artist_canonical_id) owned by a Listener with Artist Capability (and optional band_id).
• Primary genre is stored as genre_id; secondary discovery tags go into genre_tags[] for map/discovery/promotions.
• Community assignment uses the user's Home Scene (City + State + Genre) and primary genre_id to compute community_key.
• API write paths validate that the authenticated user (Listener with Artist Capability) controls the referenced artist_canonical_id.
🎯 MODULE OVERVIEW
Purpose
Comprehensive system for song upload, processing, management, and integration with the Fair Play system and Community systems. Handles the entire lifecycle of music content from upload to deletion, including audio processing, metadata management, and community assignment.
Critical Integration Points
• Fair Play system: Automatically ingests ACTIVE songs into Phase 1 (time-based) then Phase 2 (engagement-based) rotation
• Community System: Assigns songs to appropriate geographic + genre communities
• Authentication System: Manages capability permissions and ownership
...
• ❌ Current: No integration with Fair Play system
• ✅ Target: Automatic Fair Play ingestion (Phase 1 time-based rotation -> Phase 2 engagement-based rotation)
• ❌ Current: Manual community assignment
• ✅ Target: Automatic community assignment based on Home Scene (City + State + Genre)
--------------------------------------------------------------------------------
...
│   ├── Listener Dashboard (Artist Capability)
...
const UploadValidation = {
  ...
  fileLimits: {
    ...
    // Capability Slot Limits
    capabilitySlots: {
      standard: { activeSlots: 1 },
      premium: { activeSlots: 3 }
    }
  },
  ...
  statusManagement: {
    ...
    statusTypes: {
      ...
      ACTIVE: {
        ...
        systemActions: ['FAIR_PLAY_INGESTION', 'STATISTICS_TRACKING']
      },
      
      PAUSED: {
        description: 'Temporarily removed from rotation',
        userActions: ['REACTIVATE', 'EDIT', 'DELETE'],
        systemActions: ['FAIR_PLAY_REMOVAL']
      },
...
--------------------------------------------------------------------------------
🔗 INTEGRATION LAYER
Fair Play Ingestion Integration
const FairPlayIntegration = {
  // Automatic Fair Play Ingestion
  ingestion: {
    // Activation Criteria
    activationCriteria: {
      songStatus: 'ACTIVE',
      capabilitySlotAvailable: true,
      qualityScore: '>= 7.0',
      metadataComplete: '>= 80%',
    ...
    ingestionProcess: {
      ...
      5: 'PHASE_1_TIME_BASED_ROTATION',
      6: 'PHASE_2_ENGAGEMENT_BASED_ROTATION',
      7: 'CONFIRMATION_TO_USER'
    },
  ...
  // Rotation Telemetry (Recorded, Not Interpreted)
  rotationTelemetry: {
    performanceMetrics: {
...
const CommunityIntegration = {
  // Automatic Home Scene Assignment
  communityAssignment: {
    // Assignment Logic
    assignmentLogic: {
      primary: {
        source: 'USER_HOME_SCENE',
        validation: 'HOME_SCENE_AFFILIATION'
      },
      
      automatic: {
        enabled: true,
        accuracy: '>= 95%',
        fallback: 'HOME_SCENE_ONLY',
        notification: 'USER_CONFIRMATION'
      }
    },
...
1. SONG UPLOAD FLOW (Listener with Artist Capability)
Standard Upload Process
Listener Dashboard (Artist Capability) → Upload Section
...
│   ├── Genre Selection (AI-suggested + User choice)
...
├── Auto-Detection
│   ├── User Home Scene (City + State + Genre)
...
├── Automatic Fair Play Ingestion (Phase 1 -> Phase 2)
...
│   └── Active Slot Availability Check
│
...
└── Activation & Fair Play Ingestion
...
└── Re-enter Fair Play Phase 1 (If major changes)
...
const integrationTesting = {
  fairPlayIngestion: {
    tests: [
      'automatic_ingestion_on_active',
      'phase_1_time_based_rotation_entry',
      'phase_2_engagement_based_rotation_entry',
      'tier_progression_logic',
      'ingestion_telemetry_capture'
    ]
  },
...
2. Capability Interface Development
    ◦ Upload interface creation
    ◦ Metadata editing tools
    ◦ Batch operation support
Week 7-8: Integration Development
1. Fair Play Ingestion Integration
    ◦ Automatic ingestion on ACTIVE
    ◦ Phase 1 -> Phase 2 transition controls
    ◦ Rotation telemetry capture
...
• Capability user satisfaction with analytics > 4.0/5
• Support ticket volume < 2% of uploads
Integration Success
• Automatic community assignment accuracy > 95%
• Fair Play ingestion rate > 99%
• Cross-system data consistency > 99.9%
• Real-time sync success rate > 99.5%
Business Impact
• Upload volume growth > 25% quarterly
• Song quality score improvement > 10% yearly
• Community engagement increase > 20% per new song
• System scalability to 100,000+ songs
--------------------------------------------------------------------------------
This completes the comprehensive Song Management & Upload System specification. This module provides the core content creation infrastructure that feeds into the Fair Play system and Community systems, enabling listeners with Artist Capability to upload, manage, and optimize their music for the UPRISE platform.
