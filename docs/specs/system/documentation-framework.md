...
| Music-community preference runtime implementation | `docs/specs/users/onboarding-home-scene-resolution.md#music-community-preference-contract` | Runtime parity audit found current persistence/API/web paths are still single-preference compatibility paths; profile-held preference persistence, default-star semantics, roller filtering, and GPS voting scope across resolvable preferences remain implementation work. |
...
4. Implement Music-Community Preference runtime parity: profile-held preference persistence, default-star selection, current-city resolution, Home Scene roller read model, GPS voting scope across resolvable registered preferences, and migration/backfill from the current single-preference user fields.
