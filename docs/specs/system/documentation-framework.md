...
| Music-community preference runtime implementation | `docs/specs/users/onboarding-home-scene-resolution.md#music-community-preference-contract` | Persistence, backfill, API, typed web wrappers, Home Scene roller read model, and Fair Play voting across resolvable registered preferences exist; profile UI, Plot/Home roller consumption, unresolved-profile visibility, and compatibility cleanup remain implementation work. |
...
4. Continue Music-Community Preference runtime parity: profile UI, Plot/Home roller consumption, unresolved-profile visibility outside the roller, and migration cleanup after the new preference model is fully adopted.
