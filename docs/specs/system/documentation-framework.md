...
| Music-community preference runtime implementation | `docs/specs/users/onboarding-home-scene-resolution.md#music-community-preference-contract` | Persistence, backfill, API, typed web wrappers, expanded listener-profile preference management, Home Scene roller read model, Plot/Home roller consumption, and Fair Play voting across resolvable registered preferences exist; unresolved-profile visibility and compatibility cleanup remain implementation work. |
...
4. Continue Music-Community Preference runtime parity: unresolved-profile visibility outside the roller and migration cleanup after the new preference model is fully adopted.
