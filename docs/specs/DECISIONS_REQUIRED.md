...
- **Official Sect and backing authority chain**: decide whether an authorized source manager may submit a Sect-recognition request for bounded platform-admin approval, or only a platform admin may create the Official Sect before sources request affiliation. The same answer must lock requester, creator, affiliation approver/revoker, and song-backing actor; motion-first/community-role authority remains non-unblocking until separately specified.
- **Song-level Sect backing lifecycle**: preserving the settled one-active-Sect-per-song rule, decide whether closed backing relationships retain full lifecycle history or product state keeps only the current backing plus a minimal operational audit. Lock source withdrawal, Registrar invalidation with actor/time/reason, and reassignment after closure before adding backing persistence.
- **Sect readiness source diversity**: preserving the 45-minute target and existing 15-minute per-source Release Deck cap, confirm whether readiness also requires at least five distinct eligible registered sources, has no additional distinct-source minimum, or uses another explicit source minimum. Do not implement the architecture proposal as settled product truth until confirmed.
