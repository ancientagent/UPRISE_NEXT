...
- **Official Sect and backing authority chain**: decide whether Registrar-gated recognition, source-created recognition, or a motion-first process establishes an Official Sect and source affiliation strongly enough for an authorized source operator to back specific eligible songs.
- **Song-level Sect backing lifecycle**: decide whether a song may have one active backing with retained history, one overwritten current assignment, or multiple active Sect backings; lock withdrawal, invalidation, reassignment, and retention semantics before adding backing persistence.
- **Sect readiness source diversity**: confirm whether the 45-minute target also requires at least five distinct eligible registered sources with a maximum of 15 counted minutes per source, or lock a different diversity/cap rule. Do not implement the architecture proposal as settled product truth until confirmed.
