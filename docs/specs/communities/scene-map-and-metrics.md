...
- Parent context rule:
  ...
  - Current MVP Plot Archive exposure is read-only descriptive context and uses the Plot-selected `city` / `state` tier context where applicable.
  - The broader platform model may still retain `national`, but `national` is deferred from the live MVP Plot Archive until population justifies it.
...
- Plot shell includes an `Archive` tab in `apps/web/src/app/plot/page.tsx`.
- Current MVP Archive body is read-only and renders:
...
- Live Plot Archive exposure of `national` tier.
...
- Current MVP Archive tab renders read-only Top Songs + Scene Activity Snapshot modules.
- Future Scene Map / expanded Archive tiers update scope while preserving parent context.
...
- Archive descriptive values change by active Plot scope only; current MVP UI does not expose the interactive Scene Map panel as the Archive body.
- City map sect/local detail and state macro rollups remain future expanded Scene Map behavior.
