...
## [Unreleased]
### Added
- Added a browser-native Midjourney avatar picker under `tools/midjourney-avatar-picker/` that overlays `Pick avatars` controls on Midjourney Create result cards, lets you select/crop individual avatar tiles, includes a `Magic Wand` auto-fit pass for uneven sheets, opens the picker from the floating launcher on the first visible sheet, and exports PNG crops plus a JSON manifest; daily-use launcher: `pnpm run browser:mcp:launch:wsl:avatars`. See `docs/handoff/2026-03-26_midjourney-avatar-picker.md`.
