...
- Clarified UPRISE Codex-first audit/review routing so routine branch checks, stale/fixed-work checks, changed-file reviews, packet sanity, and test-output summaries use Codex by default, with GPT-5.3 Codex Spark for light work, GPT-5.5 high for normal review, GPT-5.5 extra-high for heavy audits, `uprisewatchdog` as the MiniMax/minimal-tool heartbeat lane, and Hermes basic/heavy lanes kept as fail-closed specialist or final-gate fallbacks; see `docs/handoff/2026-07-01_uprise-hermes-heavy-light-routing.md`.
