...
- Clarified UPRISE Codex-first audit/review routing so routine branch checks, stale/fixed-work checks, changed-file reviews, packet sanity, test-output summaries, PM closeout review, and final safety gates use Codex by default, with GPT-5.3 Codex Spark for light work, GPT-5.5 high for normal review, GPT-5.5 extra-high for heavy audits, and Hermes kept only as an explicit manual fallback when PM names Hermes-specific value; see `docs/handoff/2026-07-01_uprise-hermes-heavy-light-routing.md`.
- Added UPRISE Hermes manual-fallback reviewer and auditor routing: GLM-backed `uprisereviewer+` / `upriseauditor+` heavy fallback opinions, MiniMax-backed `uprisereviewer-` / `upriseauditor-` basic evidence passes, repo-local fail-closed `just` shortcuts, and a local Codex skill; see `docs/handoff/2026-07-01_uprise-hermes-heavy-light-routing.md`.
