...
## [Unreleased]
### Added
- Closed the remaining unsigned `/discover` Home Scene tuple gap by resolving valid `{city, state, music community}` onboarding context into a city-scene id before local Discover stays locked, opened anonymous current-community search/highlights reads in the Discover API/client, added regression coverage in `apps/web/__tests__/discover-page-lock.test.ts`, `apps/web/__tests__/discovery-client.test.ts`, and `apps/api/test/communities.discovery.controller.test.ts`, and recorded the slice in `docs/handoff/2026-03-24_discover-unsigned-home-scene-unlock-fix.md`.
