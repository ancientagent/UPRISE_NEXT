...
UX Batch preflight before execution:
```bash
...
node scripts/reliant-ux-preflight.mjs --queue "$QUEUE_PATH" --runtime "$RUNTIME_PATH"
```
- Batch16 required source-of-truth set:
...
- Batch17 required source-of-truth set:
...
- Preflight also validates queue/runtime naming parity for supported UX batch queues.
...
- For Batch16/Batch17 UX queues, `claim` also refuses `queue_transition_sanity_failed` when task lifecycle timestamps/statuses are impossible or queue ordering is invalid (for example `queued` with `finishedAt`, `done` without `finishedAt`, or `queued -> done`).
...
- `blocked_preflight` from `reliant-next-action` / `reliant-autopilot`
  - Cause: UX lock/spec files are missing/unreadable or the queue/runtime naming pair is wrong.
- `queue_transition_sanity_failed`
  - Cause: impossible UX batch queue lifecycle state or impossible queue edge in `scripts/reliant-slice-queue.mjs`.
...
- `blocked_only`
  - Cause: queue has blocked tasks and no queued work remaining.
- `no_queued_tasks`
  - Cause: queue is drained for the lane.
...
4. Note: `blocked_only` and `no_queued_tasks` are informational supervisor stop diagnostics; only founder-decision stop conditions exit the supervisor with code `7`.
...
### UX Batch runtime recovery
For UX lane runtime files, prefer queue-aware recovery:
```bash
RUNTIME_PATH=".reliant/runtime/current-task-lane-d-ux-batch17.json"
QUEUE_PATH=".reliant/queue/mvp-lane-d-ux-automation-batch17.json"
...
- `resumeMessage` in the JSON payload explains the exact next recovery step.
...
- Fixed transcript section order:
  ...
  - `## Verify Exit Code`
  - `## Exact Output`
- Exit metadata block now uses deterministic key-value lines:
  - `exitCode=<n>`
  - `exitSignal=<name|none>`
  - `exitStatus=passed|failed`
  - `passed=true|false`
  - `commandChainDetected=true|false`
  - `commandSegmentCount=<n>`
- Optional artifacts:
  - `--markdown-out <path>`
  - `--json-out <path>`
