...
| Core Actions (Signals: Collect/Blast/Recommend + Follow; current `ADD` row debt) | `docs/specs/core/signals-and-universal-actions.md` (`CORE-SIGNALS`), `docs/specs/social/message-boards-groups-blast.md` | Implemented signal write/action contracts: `POST /signals`, `POST /signals/:id/collect`, legacy compatibility `POST /signals/:id/add`, `POST /signals/:id/blast`, `POST /signals/:id/recommend`, `POST /follow`; idempotent action behavior and typed collection shelves. Direct signal `SUPPORT` is no longer part of the live contract. | Discourse-signal contracts and proof-of-support extensions remain future work; no algorithmic ranking semantics allowed. | Requires authenticated user context and signal/action contract integrity; downstream civic workflows remain registrar-governed. | `docs/canon/Master Narrative Canon.md`, `docs/canon/Master Glossary Canon.md` |
