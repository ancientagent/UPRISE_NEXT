...
8. **Slice 8 – Music-Community Preference Runtime Parity**
   ...
   - Preference persistence, migration backfill from `User.homeSceneCommunity`, current-user API endpoints, explicit default-star mutation, typed web wrappers, expanded listener-profile preference management, the Home Scene roller read model, Plot/Home roller consumption, and Fair Play voting across resolvable registered preferences are implemented as the runtime foundation.
   - Next: implement unresolved-profile visibility outside the roller and compatibility-field cleanup.
