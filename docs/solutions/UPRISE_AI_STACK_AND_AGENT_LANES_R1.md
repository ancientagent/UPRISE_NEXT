...
| Hermes `uprisereviewer+` / `uprisereviewer-` | Narrow post-implementation or launch review of one issue, PR, commit, deployment slice, or light packet scan | broad doctrine audits or implementation | accept/reject findings only from heavy; light returns evidence/recommendations |
| Hermes `upriseauditor+` / `upriseauditor-` | Broad read-only drift audits, documentation audits, cross-lane consistency reviews, or light evidence scans | code edits, PR ownership, provider changes | classified findings with authority evidence and follow-up issues; final blocks only from heavy |
...
Use explicit basic/heavy calls so UPRISE can save model budget without weakening final gates:

| Call | Command/profile | Model tier | Authority |
| --- | --- | --- | --- |
| `reviewer-heavy` | `uprisereviewer+` / `uprisereviewerplus` | GLM 5.2, `z-ai/glm-5.2` | Final safe/not-safe review for PR, merge, closeout, provider/db risk, owner-spec changes, branch absorption, or high-impact runtime behavior. |
...
| `auditor-heavy` | `upriseauditor+` / `upriseauditorplus` | GLM 5.2, `z-ai/glm-5.2` | Final drift/safety audit that may block branch cleanup, merge, owner-spec promotion, or closeout. |
