...
Task: Batch19 risk/rollback memo

## Scope
- Produced a severity-ordered Batch19 UX risk/rollback memo for the current state.
- Focused on residual risks after the current Batch19 lane outputs.
...
- Evidence:
  - `.reliant/queue/mvp-lane-a-ux-plot-batch19.json`
  - `docs/specs/communities/plot-and-scene-plot.md:24`
  - `docs/specs/communities/plot-and-scene-plot.md:118`
- Residual risk:
  - Batch19 lane-A blocked entries still claim the Plot spec authorizes a Social placeholder, while the current spec says Social is hidden in MVP.
...
### 2. Medium: Batch19 lane-E queue metadata still uses Batch17 titles/prompts
- Evidence:
  - `.reliant/queue/mvp-lane-e-ux-qarev-batch19.json`
- Residual risk:
...
- Evidence:
  - current lane-E `663A` completed cleanly while lane-A blocked reasons still cite an older spec contradiction
- Residual risk:
