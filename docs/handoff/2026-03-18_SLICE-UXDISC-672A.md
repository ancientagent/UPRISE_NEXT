...
## Implementation
- Added a discovery-client regression assertion proving the request query is limited to `tier`, `musicCommunity`, `state`, and `city`.
- Explicitly asserted the request never emits `artist` or `band` query params.
...
- The slice tightens the existing canon-safe boundary only.
