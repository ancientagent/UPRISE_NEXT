...
PASS __tests__/discovery-client.test.ts
  discovery client
    ...
    ✓ limits state-tier discovery queries to the approved scope keys (2 ms)
    ✓ never adds artist or band lookup params to discovery queries (1 ms)
    ...
    ✓ preserves the musicCommunity key across all discovery tiers even when the typed value trims blank (3 ms)
    ✓ limits city-tier discovery queries to the approved scope keys
    ✓ does not carry location filters into national-tier discovery
    ...
    ✓ throws when tune or set-home responses are empty (9 ms)
