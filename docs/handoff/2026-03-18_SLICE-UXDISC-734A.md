...
## Implementation
- Added a regression assertion proving fallback visitor status is preserved when the primary tuned-scene transport payload omits `isVisitor`.
- No product code path changed; the slice tightens the tested handoff contract only.
...
- Locked decisions touched: tuned-scene transport fallback only. Remains unchanged.
