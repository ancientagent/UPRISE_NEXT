...
UPRISE now preserves Hermes only as an explicit manual specialist fallback, while review/audit routing and final gates are Codex-first:

- GPT-5.3 Codex Spark: light review/audit, packet sanity, stale/fixed-work checks, changed-file skims, and test-output summaries.
- GPT-5.5 high: normal implementation review, PM closeout review, and ordinary multi-file branch review.
- GPT-5.5 extra-high: heavy audits, provider/db/security risk, branch absorption, owner-spec promotion risk, and layered/drift-prone cleanup.

The Hermes fallback lanes remain:

- `uprisereviewer+` / profile `uprisereviewerplus`: GLM 5.2 heavy manual fallback review opinion.
- `upriseauditor+` / profile `upriseauditorplus`: GLM 5.2 heavy manual fallback audit opinion.
...
Hermes lanes gather, classify, sanity-check, and recommend. They do not independently approve safety, merge, branch cleanup, owner-spec promotion, provider/db risk, or closeout. Those gates route to Codex 5.3 Spark / Codex 5.5 high / Codex 5.5 extra-high by task risk.
...
- Updated `docs/solutions/UPRISE_AI_STACK_AND_AGENT_LANES_R1.md` with the Hermes manual-fallback model-budget rule.
...
- Updated `docs/solutions/UPRISE_AI_STACK_AND_AGENT_LANES_R1.md` with the Codex model split: GPT-5.3 Codex Spark for light, GPT-5.5 high for normal, GPT-5.5 extra-high for heavy.
...
python3 "${CODEX_HOME:-$HOME/.codex}/skills/.system/skill-creator/scripts/init_skill.py" \
  ...
  --interface short_description="Route UPRISE Codex-first reviews and Hermes manual fallbacks." \
  --interface default_prompt="Use this skill when deciding whether UPRISE review/audit work should stay Codex-first or use Hermes as an explicit manual fallback. Apply basic/heavy reviewer and auditor fallback routing, prefer fresh one-shot sessions, treat /clear as a persistent-session reset, and preserve bounded read-only subagent or swarm splits."
```

Then replace the generated `SKILL.md` with the current local rules:

- Start with Codex for ordinary audits/reviews and final gates: GPT-5.3 Codex Spark for light, GPT-5.5 high for normal, GPT-5.5 extra-high for heavy.
