...
python3 "${CODEX_HOME:-$HOME/.codex}/skills/.system/skill-creator/scripts/init_skill.py" \
  ...
  --interface default_prompt="Use this skill when launching or reviewing UPRISE Hermes workers. Apply basic/heavy reviewer and auditor routing, prefer fresh one-shot sessions, treat /clear as a persistent-session reset, and preserve bounded read-only subagent or swarm splits."
```
