...
UPRISE now preserves the Hermes basic/heavy lanes as manual specialist fallbacks, while ordinary review/audit routing is Codex-first:
...
Current cost override: ordinary UPRISE audits and reviews are Codex-first. Use Hermes only when the packet names the specialist value: profile memory, owner-spec/docs specialization, QA planning state, second opinion, synthesis across worker outputs, feedback/profile-skill learning, or a heavy final safe/not-safe gate. Heavy Hermes is not a default scout.
