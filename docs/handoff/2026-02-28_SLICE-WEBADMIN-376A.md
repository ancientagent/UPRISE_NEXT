# SLICE-WEBADMIN-376A — Invite summary shape stability pack 4

## Summary
- Hardened invite summary shape assertions for queued/sent/failed mixed counters and per-entry nullable outcome/timestamp mapping.
- Scope stayed aligned to current API response behavior.
...
PASS __tests__/registrar-client.test.ts
  ...
  registrar sect-motion read client scaffolding
    ...
    ✓ calls sect-motion list endpoint and returns fallback when data is null (1 ms)
    ...
    ✓ keeps project/sect nullable mapping parity when project scene is null and sect scene is present
  ...
  registrar promoter read client scaffolding
    ...
    ✓ preserves sparse capability-audit event arrays when total exceeds materialized rows (1 ms)
    ✓ preserves sparse capability-audit payloads with additive nested metadata arrays
    ✓ preserves sparse capability-audit payloads when additive metadata contains nullable arrays (1 ms)
    ...
    ✓ throws when promoter capability-audit response data is null (3 ms)
  ...
  registrar invite-status read client scaffolding
    ...
    ✓ preserves invite summary shape when counts include queued/sent/failed mix
  ...
  registrar client auth-error propagation consistency
    ...
    ✓ propagates invalid-token auth errors consistently across list/detail and code methods
