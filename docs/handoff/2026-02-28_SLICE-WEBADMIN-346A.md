# SLICE-WEBADMIN-346A — Invite summary shape stability pack 3

## Summary
- Hardened invite summary shape assertions for additional counter variants (`pending_email`, `failed`) and per-entry nullable outcome mapping.
- Maintained scope aligned to current API response shapes only.
...
PASS __tests__/registrar-client.test.ts
  ...
  registrar project read client scaffolding
    ...
    ✓ throws when project detail response has no data payload (1 ms)
  ...
  registrar sect-motion read client scaffolding
    ...
    ✓ calls sect-motion detail endpoint and returns payload (7 ms)
    ✓ preserves nullable fields in sect-motion detail response shape
    ✓ keeps sect-motion detail contract stable when scene is present and payload contains nullable keys (1 ms)
  ...
  registrar invite-status read client scaffolding
    ...
    ✓ preserves invite summary top-level and per-entry shape stability (4 ms)
