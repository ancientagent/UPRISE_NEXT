...
PASS __tests__/plot-ux-regression-lock.test.ts
  /plot UX regression lock
    ✓ locks player mode labels to explicit RADIYO vs Collection copy (24 ms)
    ...
    ✓ locks collection mode to selection entry and explicit eject return (3 ms)
    ✓ locks engagement wheel actions to deterministic mode-specific sets (3 ms)
