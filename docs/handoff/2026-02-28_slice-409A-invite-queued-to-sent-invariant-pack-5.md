# Lane C Handoff - SLICE-INVITE-409A

Slice
Task ID: SLICE-INVITE-409A
Title: Invite queued-to-sent invariant pack 5
...
PASS test/registrar.service.test.ts
  RegistrarService
    ...
    ✓ rejects promoter registration when requester is outside Home Scene (36 ms)
    ...
    ✓ normalizes whitespace-only productionName to null in list reads (1 ms)
    ...
    ✓ rejects system-only issuance seam when registrar entry is missing (4 ms)
    ✓ rejects system-only issuance seam for non-promoter registrar entry types
    ✓ rejects system-only issuance seam when promoter entry is not approved (1 ms)
    ...
    ✓ rejects promoter approval transition when registrar entry has unknown lifecycle status (1 ms)
    ...
    ✓ rejects promoter approval transition for non-promoter registrar entry types (1 ms)
    ...
    ✓ revokes active promoter capability grant when registrar entry status is rejected (1 ms)
    ...
    ✓ returns deterministic replay-block message when an active issued code already exists (1 ms)
    ...
    ✓ retries registrar code issuance on transient codeHash uniqueness conflict and succeeds (1 ms)
    ✓ fails registrar code issuance after exhausting uniqueness retry attempts
...
Complete Command:
node scripts/reliant-slice-queue.mjs complete --queue .reliant/queue/mvp-lane-c-invite-batch14.json --runtime .reliant/runtime/current-task-lane-c-batch14.json --task-id SLICE-INVITE-409A --report docs/handoff/2026-02-28_slice-409A-invite-queued-to-sent-invariant-pack-5.md

Complete Output:
{"completed":true,"taskId":"SLICE-INVITE-409A","updatedAt":"2026-02-28T05:36:06.878Z"}
