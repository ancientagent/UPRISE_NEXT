...
## Implementation
- Added a discovery-client regression proving state-tier queries with a blank typed `state` value do not emit `state` or `city` lookup semantics.
- Added a communities-client regression proving national active-statistics responses preserve data even when no active-scene metadata is present.
...
- Touched locked decision: discovery scope remains determined only by approved tier and scene/community inputs.
  - Confirmed unchanged.
- Touched locked decision: active-scene statistics transport can preserve payload data without requiring scene metadata.
  - Confirmed unchanged.
