...
## Implementation
- Added `aria-relevant="text"` to the shared Discover summary status surface.
- This keeps the existing state model intact while tightening how summary updates are announced.
...
PASS __tests__/discovery-client.test.ts
  discovery client
    ...
    ✓ never adds artist or band lookup params to discovery queries (2 ms)
