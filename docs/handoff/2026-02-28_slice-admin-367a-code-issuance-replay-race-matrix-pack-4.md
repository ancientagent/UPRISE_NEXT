# SLICE-ADMIN-367A — Code issuance replay race matrix pack 4

## Scope
- Hardened replay/race safeguards coverage around registrar code issuance for approved entries.
- Added deterministic replay-block message assertions for pre-check and transactional race-window duplicate detection.
...
PASS test/registrar.service.test.ts
  RegistrarService
    ✓ submits promoter registration for Home Scene with named production identity (1 ms)
    ...
    ✓ rejects project registration for non city-tier scene (1 ms)
    ...
    ✓ rejects sect-motion registration when requester is outside Home Scene (1 ms)
    ...
    ✓ lists promoter capability audit events for submitter-owned registration (3 ms)
    ...
    ✓ rejects system-only issuance seam for non-promoter registrar entry types (1 ms)
    ...
    ✓ emits consistent system actor metadata on both approve and reject audit events (1 ms)
    ...
    ✓ rejects promoter approval transition from submitted to unsupported target status literal (1 ms)
    ...
    ✓ revokes active promoter capability grant with append-only audit log emission (4 ms)
    ...
    ✓ rejects promoter capability revocation when active grant-link entry differs from target entry (1 ms)
    ...
    ✓ rejects registrar code issuance replay when an active issued code already exists (1 ms)
    ✓ returns deterministic replay-block message when an active issued code already exists
    ✓ rejects registrar code issuance when duplicate active code appears during transactional race window
    ✓ returns deterministic replay-block message for transactional race-window duplicate detection (1 ms)
    ...
    ✓ rejects registrar code verification when code does not exist (3 ms)
    ✓ rejects registrar code verification when code is no longer redeemable (1 ms)
...
## Queue Transition
```text
{"completed":true,"taskId":"SLICE-ADMIN-367A","updatedAt":"2026-02-28T05:13:49.650Z"}
```
