...
- Confirmed the current Batch21 player/profile implementation satisfies the locked route, player, and expanded-profile contracts.
- Added an explicit regression assertion proving `/plot` wires `playerMode` into `RadiyoPlayerPanel` via `mode={playerMode}`.
