...
## Implementation
- Added a discovery-client regression assertion proving blank typed `musicCommunity` input still results in only the approved query keys.
- Explicitly asserted the query does not introduce `artist` or `band` params in that edge case.
