# SLICE-WEBADMIN-286A — Invite summary shape assertions (top-level + per-entry)

## Summary
- Added invite summary shape-stability assertions for top-level response structure and per-entry outcome fields.
- Coverage remains aligned to current API behavior and stays test-only.
...
PASS __tests__/registrar-client.test.ts
  ...
  registrar promoter read client scaffolding
    ...
    ✓ throws when promoter capability-audit response data is null (6 ms)
