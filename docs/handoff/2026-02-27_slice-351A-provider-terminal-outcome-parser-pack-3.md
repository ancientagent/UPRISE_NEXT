# Lane C Handoff - SLICE-INVITE-351A

Slice
Task ID: SLICE-INVITE-351A
Title: Provider terminal-outcome parser pack 3
...
PASS test/registrar.service.test.ts
  RegistrarService
    ...
    ✓ rejects promoter registration when requester is outside Home Scene (42 ms)
    ...
    ✓ rejects project registration when requester user is missing (1 ms)
    ...
    ✓ rejects sect-motion registration when requester user is missing (1 ms)
    ...
    ✓ aggregates promoter list counts across mixed statuses (1 ms)
    ...
    ✓ emits deterministic audit ordering for approve-then-issue lifecycle helper (4 ms)
    ✓ rejects approve-then-issue lifecycle helper when actor context is not system (1 ms)
    ...
    ✓ rejects approve-then-issue lifecycle helper when issuance-phase entry is missing (1 ms)
    ...
    ✓ applies promoter approval transition from submitted to rejected (1 ms)
    ...
    ✓ rejects promoter approval transition matrix from approved to rejected (1 ms)
    ...
    ✓ normalizes approval/rejection reason payload to bounded stable metadata shape (1 ms)
    ...
    ✓ rejects promoter capability revocation when grant state is non-active (1 ms)
    ✓ rejects promoter capability revocation when grant has revokedAt timestamp
    ...
    ✓ throws when target scene does not exist (1 ms)
    ...
    ✓ materializes submitted entry into ArtistBand + owner membership (2 ms)
    ...
    ✓ rejects invite dispatch from non-submitting user (1 ms)
    ✓ finalizes queued invite delivery as sent
    ✓ rejects invite delivery finalization when delivery row is missing
    ✓ returns already-finalized response when delivery is no longer queued (1 ms)
    ...
    ✓ rejects invite status read when registrar entry is missing (1 ms)
    ...
    ✓ syncs claimed/existing registrar members into canonical artist-band memberships (1 ms)
...
Complete Command:
node scripts/reliant-slice-queue.mjs complete --queue .reliant/queue/mvp-lane-c-invite-batch12.json --runtime .reliant/runtime/current-task-lane-c-batch12.json --task-id SLICE-INVITE-351A --report docs/handoff/2026-02-27_slice-351A-provider-terminal-outcome-parser-pack-3.md

Complete Output:
{"completed":true,"taskId":"SLICE-INVITE-351A","updatedAt":"2026-02-28T02:45:53.435Z"}
