...
Task: Discovery card metadata consistency pass
Status: completed
...
## Implementation
- Normalized city-card scope metadata from `City Scene` to `City`.
- Normalized state-card scope metadata from `State Rollup` to `State`.
- Shortened both card metadata labels from `Music Community` to `Community` so the result cards use one consistent metadata vocabulary.
...
- No CTA, entitlement, or speculative behavior changes were added.
- Touched locked decision: Discover result cards remain deterministic metadata surfaces, not explanatory marketing copy.
  - Confirmed unchanged.
