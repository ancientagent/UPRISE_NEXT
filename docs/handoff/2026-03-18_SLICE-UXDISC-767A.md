...
## Implementation
- Added explicit typed-client assertions for `POST /discover/tune` and `POST /discover/set-home-scene`.
- Verified both mutations send the expected `sceneId` payload and auth headers.
...
- Locked decisions touched: explicit discovery mutation contract only. Remains unchanged.
