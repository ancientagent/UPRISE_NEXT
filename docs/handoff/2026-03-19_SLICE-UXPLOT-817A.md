...
PASS __tests__/plot-ux-regression-lock.test.ts
  /plot UX regression lock
    ✓ locks player mode labels to explicit RADIYO vs Collection copy (5 ms)
    ✓ locks panel-state ownership to the /plot route container (1 ms)
    ✓ locks compact player shell scaffolding for track row and tier stack
    ✓ locks collection mode to selection entry and explicit eject return
    ✓ locks engagement wheel actions to deterministic mode-specific sets
