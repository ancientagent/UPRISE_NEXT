...
## Implementation
- Reduced city-scene status chip wording to the fixed metadata labels `Active` and `Inactive`.
- Preserved the existing `Home Scene` and `Tuned Scene` labels.
...
- The slice only normalizes existing card metadata labels.
