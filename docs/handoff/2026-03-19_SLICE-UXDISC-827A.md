...
## Implementation
- Added a communities-client regression proving national active-statistics requests use the approved `tier=national` query shape only.
- This closes the lane with an explicit scope assertion for the active-scene fallback contract that Discovery hands off into Plot.
...
- Touched locked decision: discovery/plot fallback reads remain scope-driven and deterministic at national tier.
  - Confirmed unchanged.
