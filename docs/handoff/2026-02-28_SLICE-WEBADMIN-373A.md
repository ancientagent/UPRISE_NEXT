# SLICE-WEBADMIN-373A — Registrar client auth error consistency pack 4

## Summary
- Added focused invalid-token auth-error propagation parity checks across list/detail reads and verify/redeem methods.
...
PASS __tests__/registrar-client.test.ts
  registrar client scaffolding
    ...
    ✓ throws when registrar code verify response has no data payload (10 ms)
    ✓ calls registrar code redeem endpoint and returns typed payload
    ✓ throws when registrar code redeem response has no data payload (1 ms)
    ✓ propagates forbidden error from registrar code verify endpoint
  ...
  registrar sect-motion read client scaffolding
    ...
    ✓ calls sect-motion list endpoint and returns fallback when data is null (7 ms)
    ...
    ✓ keeps project/sect nullable mapping parity for mixed null/non-null payload keys
  ...
  registrar promoter read client scaffolding
    ...
    ✓ preserves sparse capability-audit payloads with additive nested metadata arrays (1 ms)
  ...
  registrar invite-status read client scaffolding
    ...
    ✓ keeps queued/claimed invite outcomes with stable nullable field mapping (5 ms)
  ...
  registrar client auth-error propagation consistency
    ...
    ✓ propagates token-expired auth errors consistently across read and code methods
    ✓ propagates invalid-token auth errors consistently across list/detail and code methods (1 ms)
