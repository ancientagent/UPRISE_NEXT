# SLICE-WEBADMIN-255A — Registrar invite read contract nullability normalization

## Summary
- Normalized invite read contract coverage for `queued` and `claimed` outcomes with stable nullable field mapping assertions.
- Added focused tests only; no UI actions/CTAs and no API behavior expansion.
