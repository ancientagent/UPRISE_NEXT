...
PASS __tests__/plot-ux-regression-lock.test.ts
  /plot UX regression lock
    ✓ locks player mode labels to explicit RADIYO vs Collection copy (4 ms)
    ✓ locks panel-state ownership to the /plot route container (1 ms)
    ✓ locks compact player shell scaffolding for track row and tier stack (2 ms)
    ✓ locks collection mode to selection entry and explicit eject return (2 ms)
    ✓ locks engagement wheel actions to deterministic mode-specific sets (2 ms)
    ✓ locks expanded-profile behavior to swap out Plot tabs/body (2 ms)
    ✓ locks Top Songs + Scene Activity to statistics-only placement (1 ms)
    ✓ locks primary Plot tab ownership to explicit Feed/Events/Promotions/Statistics bodies (1 ms)
    ✓ locks feed copy to scene-scoped deterministic, non-personalized states (1 ms)
