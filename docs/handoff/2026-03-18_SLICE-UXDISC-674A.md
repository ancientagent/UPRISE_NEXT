...
## Implementation
- Hardened `mergeDiscoveryContextPatch()` so a primary tuned-scene object supplies its own `id` when `tunedSceneId` is omitted.
- Added a regression test covering that partial primary payload shape.
...
- The slice stays limited to deterministic transport-context patching.
