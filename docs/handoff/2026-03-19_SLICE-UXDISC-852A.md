...
## Implementation
- Added a focused typed-client regression that exercises city, state, and national discovery requests with a blank typed music-community value.
- Locked all three query shapes to preserve the `musicCommunity` key while explicitly asserting that `artist` and `band` params never appear.
