...
That assertion depends on `SLICE-UXPLOT-606A`, which is blocked because:
- `docs/specs/communities/plot-and-scene-plot.md` still says Social remains a placeholder until the endpoint ships, while
- `docs/solutions/MVP_UX_MASTER_LOCK_R1.md` and Batch18 task `SLICE-UXPLOT-606A` require no Social exposure.
