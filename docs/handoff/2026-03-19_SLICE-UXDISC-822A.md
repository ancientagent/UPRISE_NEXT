...
## Implementation
- Added a focused typed-client regression that exercises city, state, and national discovery requests with blank optional location values.
- Locked all three query shapes to the approved scene/community scope keys and explicitly asserted that `artist` and `band` params never appear.
