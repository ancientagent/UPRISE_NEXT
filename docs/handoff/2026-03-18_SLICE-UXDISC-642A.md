...
## Implementation
- Bound the Discover Music Community field to an explicit helper note via `htmlFor`, `id`, and `aria-describedby`.
- Locked the helper copy to MVP-safe semantics: Scene and Music Community scope only, with artist/band lookup explicitly unsupported.
...
- No new CTA or speculative behavior added.
- No artist/band search behavior introduced.
