...
| feed-card-family-inventory | branch | docs/feed-card-family-inventory | /home/baris/UPRISE_NEXT | - | main | d4dc636 | active | Codex local | Codex local; Codex read-only subagents for UX_UI, ACTIONS_SIGNALS, EVENTS_ARCHIVE | Task 9: Feed card family inventory. Inventory current Feed runtime cards and classify launch/beta/source/remove; optional small regression locks only, no new runtime actions. | 2026-07-03 | open PR after inventory, validation, and review if tests/runtime change |
