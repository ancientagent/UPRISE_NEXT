...
| plot-profile-player-state-contract | branch | test/plot-profile-player-state-contract | /home/baris/UPRISE_NEXT | - | main | pending | active | Codex local | Codex local | Add current-compatible Plot profile/player state contract tests inspired by preserved UX reference state-machine work; no prototype runtime import. | 2026-07-02 | merge tests/docs via PR if green |
